...
export default function FolderView({
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Sort controls */}
      <div className="flex items-center gap-2 px-4 py-2 border-b border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-850 shrink-0">
        <div className="flex items-center gap-1 rounded-lg bg-surface-100 dark:bg-surface-800 p-0.5">
          <button
            className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-colors ${
              viewMode === 'list'
                ? 'bg-white dark:bg-surface-750 text-surface-800 dark:text-surface-200 shadow-sm'
        ...
        <div className="relative">
          <button
            className="notion-button-ghost h-7 text-xs gap-1.5"
          ...
          {sortMenuOpen && (
            <div
              className="absolute left-0 top-full mt-1 z-50 min-w-[160px] rounded-lg border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 shadow-lg py-1"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 p-4">
              {sortedItemIds.map((itemId) => {
                ...
                return (
                  <button
                    key={item.id}
                    className="notion-card p-4 text-left flex flex-col gap-2 group"
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      {item.isFavorite && (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-yellow-500 shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="divide-y divide-surface-200 dark:divide-surface-700">
              {sortedItemIds.map((itemId) => {
                ...
                return (
                  <button
                    key={item.id}
                    className="flex items-center gap-3 w-full px-4 py-3 text-left hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors group"
                    onClick={() => onSelectItem(item.id)}
                    onContextMenu={(e) => handleContextMenu(e, item.id)}
                  >
          ...
          <div className="notion-empty-state h-full">
            ...
            {isCreatingItem && (
              <div className="flex items-center gap-2 mt-4">
                <input
                  data-new-item-input
                  className="notion-input h-8 text-xs w-56"
